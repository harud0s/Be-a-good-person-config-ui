...
export default function App() {
  ...
  const handleSaveRoot = async (data: any) => {
    try {
      await saveFile(data);
      toast.success(`已成功儲存 ${activeFile?.name}`);
    } catch (e) {
      toast.error('儲存失敗');
    }
  };
  ...
  const handleSaveItem = async (data: any) => {
    if (mainArrayKey && editingItemIndex !== null) {
      ...
      try {
        await saveFile({ ...activeFileContent, [mainArrayKey]: newArray });
        toast.success(`已儲存項目`);
  ...
  const handleDirtyChange = (dirty: boolean) => {
    if (activeFileContent) {
      updateActiveContent(activeFileContent, dirty);
    }
  };

  const handleOpenFile = (file: any) => {
    if (isDirty && !window.confirm("您有未儲存的變更，確定要放棄嗎？")) return;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      <div className="flex flex-1 overflow-hidden relative">
                  ...
                  {files.map((file) => (
                    ...
                    >
                      <div className="flex items-center gap-2 font-medium">
                        ...
                        {file.name}
                        {activeFile?.name === file.name && isDirty && <span className="w-2 h-2 rounded-full bg-orange-500 ml-auto"></span>}
                      </div>
          ...
          {!activeFile ? (
            ...
            <div className="max-w-6xl mx-auto pb-24">
              <div className="mb-6 pb-4 border-b">
                ...
                <p className="text-xs mt-2 opacity-50">Version: {meta?.version}</p>
              </div>
              ...
              {mainArrayKey ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {activeFileContent[mainArrayKey].map((item: any, index: number) => {
                    const title = item.event_name || item.display_name || item.goal_name || item.card_name || item.question || item.exam_name || item.id || `Item ${index}`;
                    const subtitle = item.event_id || item.goal_id || item.card_id || item.exam_id;
                    return (
                      <div 
                        ...
                        onClick={() => {
                          if (isDirty && !window.confirm("有未儲存的變更，確定要放棄嗎？")) return;
                          setEditingItemIndex(index);
                          updateActiveContent(activeFileContent, false); // clear dirty state for new edit
                        }}
                      >
        ...
        {/* 編輯抽屜 (Drawer/Sheet) */}
        {editingItemIndex !== null && mainArrayKey && (
            ...
            <div className="flex items-center justify-between p-4 border-b bg-background sticky top-0 z-10">
              ...
              <button 
                onClick={() => {
                  ...
                  setEditingItemIndex(null);
                  updateActiveContent(activeFileContent, false);
                }} 
